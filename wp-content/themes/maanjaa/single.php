...
<?php 
	...
	if ( ! empty( $categories ) ) {
		foreach( $categories as $category ) {
		    $cat_output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', 'maanjaa' ), $category->name ) ) . '">' . esc_attr( $category->name ) . '</a>' . $separator;
		}
	}
	// variables
